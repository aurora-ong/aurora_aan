...
public class AANProtocol2 extends ProtocolHandler<PeerController> {

    private static final Logger log = LoggerFactory.getLogger(AANProtocol2.class);

    public static final String announce = "/aurora/aan/0.1.0";

    BehaviorSubject<List<p2pPeerNode2>> networkPeers;

    AANSerializer annSerializer;

    public AANProtocol2(BehaviorSubject<List<p2pPeerNode2>> networkPeers, AANSerializer annSerializer) {
...
//        return chatController.connectionStatus.asObservable().filter(p2PPeerConnectionStatus -> p2PPeerConnectionStatus == P2PPeerConnectionStatus.CONNECTED).toBlocking().
    ...
    protected CompletableFuture<PeerController> onStartResponder(@NotNull Stream stream) {
        ...
        return ready.thenApply(peerController -> {
            Optional<p2pPeerNode2> nodePeerOptional = networkPeers.getValue().stream().filter(p2PPeerNode -> Objects.equals(p2PPeerNode.peerId, stream.remotePeerId().toString())).findFirst();

            if (nodePeerOptional.isEmpty()) {
                stream.close();
                throw new RuntimeException("El nodo no está registrado en esta blockchain");
            }
            p2pPeerNode2 nodePeer = nodePeerOptional.get();
